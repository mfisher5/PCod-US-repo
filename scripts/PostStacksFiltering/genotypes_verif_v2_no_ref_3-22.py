...
###       can change the min stack depth if wanted                                                                                                 - lines 60 62 64 108 110 112


##MF edited 12/16/2016
## STACK DEPTH OF 5 --- CHANGED ALL "9"S TO "4"SAVES
##########################################################################################
#python genotypes_verif_v2_no_ref.py


												## Open the output genotype file for writing
genotype_file=open('/mnt/hgfs/Pacific cod/DataAnalysis/PCod-US-repo/stacks_wgenome/batch_1.CorrectedGenotypes_biallelic.txt' ,'w')
...
Catalog_file=open('/mnt/hgfs/Pacific cod/DataAnalysis/PCod-US-repo/stacks_wgenome/batch_1.biallelic_catalog.tsv','r')
...
list_of_indiv=open('/mnt/hgfs/Pacific cod/DataAnalysis/PCod-US-repo/stacks_wgenome/CorrectedGenos_Individs.txt','r')
for indiv in list_of_indiv:						## For each string in the file
	...
	matches_file=open('/mnt/hgfs/Pacific cod/DataAnalysis/PCod-US-repo/stacks_wgenome/'+ indiv_name +'.matches.tsv','r')	# Open the matches.tags file for each individual in indiv_name list
	ind_loc_vs_cat={}																		# Start a dictionary that will hold some info????
	cat_loc=[]
	next(matches_file)
	...
	list_ind_loci=ind_loc_vs_cat.keys()														# The list_in_loci become the keys to the dictionary you initialized
	#ind_loc_vs_cat = [int(x) for x in ind_loc_vs_cat]
	#ind_loc_vs_cat.sort()
#	print ind_loc_vs_cat
	matches_file.close()
	#print ind_loc_vs_cat['1']
	Indiv_file=open('/mnt/hgfs/Pacific cod/DataAnalysis/PCod-US-repo/stacks_wgenome/'+ indiv_name +'.tags.tsv','r')	
