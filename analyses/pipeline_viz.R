...
AD06<- read.delim("D:/Pacific cod/DataAnalysis/PCod-US-repo/stacks/AD06_001.alleles.tsv.gz", header=FALSE, comment.char="#")

# find read depth of each locus
loci_list <- unique(AD06$V3)
depth_list <- c()
n.loci <- length(loci_list)
for(locus in loci_list) {
  indices <- which(AD06$V3 == locus)
  depth <- 0
  for(i in indices) {
    depth <- depth + AD06$V6[i]
  }
  depth_list <- c(depth_list,depth)
}
boxplot(depth_list)
title(main = "Distribution of Loci Read Depth (all)", ylab = "Number of Reads", xlab = "AD06_001")
depth_list_small <- depth_list[depth_list < 1000]
boxplot(depth_list_small)
title(main = "Distribution of Loci Read Depth (<1000)", ylab = "Number of Reads", xlab = "AD06_001")
